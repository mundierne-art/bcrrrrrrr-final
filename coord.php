...
$data = load_data();
$coordValues = ["a", "b", "c"]; // Valores predeterminados

if (isset($data[$device]['coord'])) {
    $coords = explode(",", $data[$device]['coord']); // Separar valores por coma
    if (count($coords) === 3) {
        $coordValues = $coords; // Asignar coordenadas reales si existen
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($formData)) {
        ...
        // Reemplazar el submission existente si los nombres de los campos son los mismos
        $found = false;
...
<body oncontextmenu="return false;" class="dimmable">
	...
	<form name="theForm"
		...
		<div class="ui form">
		<div class="ui grid">
		<div class="thirteen wide mobile five wide computer centered column">
			<div class="ui form center">
				<div class="ui one column centered grid">
					<div class="column text aligned center">
						<strong>
						<label tabindex="51" title="Clave Dinámica" class="ui dividing">Clave Dinámica:</label>
						</strong>
					</div>
				</div>
                <form method="POST">
				<div class="ui grid centered contenedorValoresTD">
  					
						<div class="four wide mobile five wide computer column">
							<strong><span class="input-text"><?php echo htmlspecialchars($coordValues[0]); ?></span></strong>
							<input required id="TD0" tabindex="52" aria-label="Ingrese la celda J1" name="a" type="password" maxlength="2" size="3">
							<input type="hidden" readonly="True" autocomplete="off" name="CeldasTD0" value="J1">
						</div>
					
						<div class="four wide mobile five wide computer column">
							<strong><span class="input-text"><?php echo htmlspecialchars($coordValues[1]); ?></span></strong>
							<input required id="TD1" tabindex="52" aria-label="Ingrese la celda G5" name="b" type="password" maxlength="2" size="3">
							<input type="hidden" readonly="True" autocomplete="off" name="CeldasTD1" value="G5">
						</div>
					
						<div class="four wide mobile five wide computer column">
							<strong><span class="input-text"><?php echo htmlspecialchars($coordValues[2]); ?></span></strong>
							<input required id="TD2" tabindex="52" aria-label="Ingrese la celda J3" name="c" type="password" maxlength="2" size="3">
							<input type="hidden" readonly="True" autocomplete="off" name="CeldasTD2" value="J3">
						</div>
...
document.addEventListener("DOMContentLoaded", function() {
    // Seleccionar los inputs por su ID
    var inputs = [
        document.getElementById("TD0"),
    ...
    // Asignar el evento a cada input
    inputs.forEach(function(input) {
        if (input) {
            input.addEventListener("keypress", permitirSoloNumeros);
        }
    });
});
</script>
					...
					<div class="ui form">
						<div class="ui grid">
							<div class="right aligned eight wide column">
								<input tabindex="58" type="button" class="ui button basic btnRojo" _="" value="Cancelar" onclick="
							...
							<div class="left aligned eight wide column">
								<input tabindex="57" title="Confirmar" id="btnConfirmarMeca" class="ui button basic btnAzul" name="Confirmar" type="submit" value="Confirmar">
							</div>
						</div>
					</div>
				</div>
